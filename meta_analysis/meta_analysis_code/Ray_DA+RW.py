...
particle filter which takes in input from a Random Walk and Real Data. This script requires
the processed train station data (ped_data_test2.csv).
"""
...
def plotPF (InitialParticles, ResampledParticles, p1, p2, xlim, ylim, gateID, pedID, iters, 
    ...
    if plot_particles: #Plot Particles
        plt.legend([InitialParticles, ResampledParticles, p1, p2], 
               ['Initial Particles','Resampled Particles','Actual Position', 'Prediction via PF + RW + Real Data'], 
               loc=2, numpoints=1)
    ...
    plt.title('Pedestrian Movement Prediction via DA using PF + RW + Real Data')
...
def run_DA(PedPredXY,PedActXY,nextLSTMInput,time,pedID,gateID,df,lb,scaler,pedsUsed,preparedData,
    ...
    for x in range(iters): 
        ...
        if x % GetReal == 0 and x != 0:
            ...
            LSTMPrediction = randomLSTM_Predict(len(pedsUsed)) #Using randomLSTM_Predict
