...
This script predicts pedestrian movement using only LSTM input. There is no input of real
data in this model. This script requires the processed train station data 
(ped_data_test2.csv) and the LSTM Model to function.
"""
import numpy as np
import pandas as pd
...
"""
Returns the Pedestrian's Predicted X and Y Location in a [x,y] format using the 
prediction array as a input
"""
...
def plotLSTM (p1, p2, xlim, ylim, gateID, pedID, iters, savePlot):          
    ...
    plt.title('Pedestrian Movement Prediction via LSTM Only')
...
#Function which runs the LSTM Model:
def run_LSTM(PedPredXY,PedActXY,nextLSTMInput,time,pedID,gateID,df,lb,scaler,pedsUsed,preparedData,
            iters=50, 
    ...
    for x in range(iters):        
        ...
        #Plot the predicted positions using LSTM Only:
...
#Get list of pedIDs and gate_outs at time 
data_now = df[df['Time']==time]
ped_IDs_now = pd.unique(data_now['ID'])
gate_IDs_now = data_now['Gate_out'].to_numpy()

#Set GateID and PedID (Example PedID = 7, GateID = 0) to Analyse:
gateID = gate_IDs_now[0]
pedID = ped_IDs_now[0]
#----------------------------------------------------------------------------
...
while q < (len(ped_IDs_now)):

    #Update pedID and gateID:
    gateID = gate_IDs_now[q]
    pedID = ped_IDs_now[q]
    
    #0.0 Print the Current PedID and GateID
    print("------------------------")
    ...
    """
    5.0 Run the LSTM Model
    - iters = Number of iterations
	- plot_iterations = if TRUE plots each iteration of the LSTM Model good for GIFs.
    ...
    try:
        ...
        print("There was not enough data in PedID: " + str(pedID))
    pass
